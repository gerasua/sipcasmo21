...
class operadorUpdateCliente(QDialog):
    def __init__(self, idItem, nombreItem, parent=None):
        super(operadorUpdateCliente, self).__init__(parent, flags=Qt.WindowCloseButtonHint)
        loadUi('Operador_Cliente_Update.ui', self)
        logger.debug("Ventana actualizar")
        self.DB_Connect()
        logger.debug("Id: {}".format(idItem))
        logger.debug("Nombre Item: {}".format(nombreItem))

        self.cur.execute("SELECT * FROM totalco_cliente WHERE id_cliente='" + idItem + "'")
        self.row = self.cur.fetchone()

        self.claveCliente = self.row[1]
        ...
        self.id = idItem
        self.lineEditUpdateClave.setText(str(self.claveCliente))
        ...
        self.btnActualizarCliente.clicked.connect(self.actualizar)
    ...
    def actualizar(self):
        ...
        self.claveClienteUpdate = self.lineEditUpdateClave.displayText()
        ...
        if self.itemUpdate:
            ...
            try:
                query = """ UPDATE totalco_cliente SET nombre_cliente = %s, clave_cliente = %s, clave_cliente_sap = %s WHERE id_cliente = %s """
                data = (str(self.itemUpdate), str(self.claveClienteUpdate), str(self.codigoSapUpdate), self.id)
