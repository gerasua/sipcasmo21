...
class reimprimirMovIntDestare(QDialog):
    def __init__(self, ticketres, parent=None):
        super(reimprimirMovIntDestare, self).__init__(parent, flags=Qt.WindowCloseButtonHint)
        loadUi('Operador_Reimprime_Destare_MovInt.ui', self)
        logger.debug("Operador_Reimprime_Destare_MovInt")
        logger.debug("Operador re imprime ticket de Movimiento Interno Destare")
        self.DB_Connect()
        ...
        try:
            self.cur.execute("SELECT origen, operador_entrada, producto, fecha_primera, hora_primera, usuario_primera, observaciones_entrada, placas, primera_pesada, segunda_pesada, peso_neto_embalaje, fecha_segunda, hora_segunda,usuario_segunda, destino, observaciones_salida, operador_salida, total_embalaje, peso_neto_sin_embalaje_redondeo, fecha_destare, hora_destare, observaciones, usuario_desatara, peso_neto_sin_embalaje FROM totalco_destare WHERE numero_ticket ='" + str(self.NumeroTicket) + "'")
    ...
    def insertarDatos3(self):
        ...
        if self.impresora == "Impresora de Cinta":
            ...
            try:
                ...
                printer.text("Origen: {}".format(self.origen))
                printer.text("Destino: {}".format(self.destino))
                printer.text(f"Producto: {str(self.producto)}")
                printer.text(f"Placas: {str(self.placas_insert)}")
                printer.text("Primera Pesada: {}".format(self.pesoEntrada))
                printer.text("Fecha/Hora: {}".format(self.EFechaHora))
                printer.text(f"Segunda pesada: {int(self.pesoTara)}")
                printer.text(f"Fecha/Hora: {str(self.SFechaHora)}")
                printer.text(f"Peso Neto: {int(self.pesoNeto)}")
                ...
                try:
                    ...
                    logger.debug(f"Error con la linea ____")
    ...
    def insertarDatos4(self):
        ...
        #Fecha y Hora Segunda Pesada
        self.fechahorasegunda = self.SFecha.displayText()
        logger.debug(f"Fecha/Hora Segunda: {self.fechahorasegunda}")
        ...
        #         printer.text("RE IMPRESION")
        #         printer.text("Segunda Pesada: {}".format(int(self.pesoTara)))
        #         printer.text(f"Peso Neto: {self.pesoNeto}")
        #         printer.text(f"Fecha/Hora: {str(self.SFechaHora)}")
        #         printer.text("Chofer: {}".format(self.operador_salida))
        #         printer.text("Pesador: {}".format(self.usuario_segunda))
        #         printer.text("Destino: {}".format(self.destino))
