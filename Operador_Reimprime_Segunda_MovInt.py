...
class reimprimirMovIntSegunda(QDialog):
    def __init__(self, ticketres, parent=None):
        super(reimprimirMovIntSegunda, self).__init__(parent, flags=Qt.WindowCloseButtonHint)
        loadUi('Operador_Reimprime_Segunda_MovInt.ui', self)
        logger.debug("Operador_Reimprime_Segunda_MovInt")
        logger.debug("Operador re imprime ticket de Movimiento Interno 2a Pesada")
        self.DB_Connect()
        ...
        try:
            self.cur.execute("SELECT pri.origen AS Origen, opee.nombre AS ChoferE, prod.nombre_producto AS Producto, pri.fecha_e AS FEntrada, pri.hora_e AS HEntrada, usere.nombre_usuario AS PesadorE, pri.observaciones AS ObservacionesE, trac.placas AS Placas, pri.peso_e AS PrimeraPesada, seg.peso_tara AS SegundaPesada, seg.peso_neto AS PesoNeto, seg.fecha_s AS FSalida, seg.hora_s AS SHora, users.nombre_usuario AS PesadorS, seg.destino AS Destino,seg.observaciones AS ObservacionesS, opes.nombre AS ChoferS FROM totalco_boleto_ent pri INNER JOIN totalco_operadores opee ON (pri.id_operador = opee.id_operador) INNER JOIN totalco_producto prod ON (pri.id_producto = prod.id_producto) INNER JOIN usuarios usere ON (pri.id_usuario = usere.id_usuario) INNER JOIN totalco_tractor trac ON (pri.id_tractor = trac.id_tractor) INNER JOIN totalco_boleto_sal seg ON (pri.id_bol_entra = seg.id_bol_sal) INNER JOIN usuarios users ON (seg.id_usuario = users.id_usuario) INNER JOIN totalco_operadores opes ON (seg.id_operador = opes.id_operador) WHERE pri.id_bol_entra ='" + str(self.NumeroTicket) + "'")
            self.row = self.cur.fetchone()

            self.EFechaHora = str(self.row[3]) + " " + str(self.row[4])
            self.origen = self.row[0]
            ...
            self.usuario_segunda = self.row[13]
            self.destino = self.row[14]
            self.observaciones_salida = self.row[15]
            self.operador_salida = self.row[16]

        except Exception as e:
            logger.debug("Error: {}".format(e))

        self.Origen.setText(str(self.origen))
    ...
    def insertarDatos(self):
        ...
        if self.impresora == "Impresora de Cinta":
            ...
            try:
                ...
                printer.text("Movimiento Interno")
                printer.text("RE IMPRESION")
                printer.text("Boleto # {}".format(self.NumeroTicket))
                printer.text("Origen: {}".format(self.origen))
    ...
    def insertarDatos2(self):
        ...
        if self.impresora == "Impresora de Cinta":
            ...
            item_widget = []
            try:
                datos += f"<tbody><tr><td>&nbsp;</td><td>&nbsp;</td></tr></tbody><tbody><tr><td>&nbsp;</td><td>&nbsp;</td></tr></tbody><tbody><tr><td>&nbsp;</td><td>&nbsp;</td></tr></tbody><tbody><tr><td>&nbsp;</td><td>&nbsp;</td></tr></tbody><tbody><tr><td>&nbsp;</td><td>&nbsp;</td></tr><tbody><tbody><tr><td>&nbsp;</td><td>&nbsp;</td></tr></tbody><tbody><tr><td>&nbsp;</td><td>&nbsp;</td></tr></tbody><tbody><tr><td>&nbsp;</td><td>&nbsp;</td></tr></tbody><tbody><tr><td>&nbsp;</td><td>&nbsp;</td></tr></tbody><tbody><tr><td>&nbsp;</td><td>&nbsp;</td></tr></tbody><br><br><br><thead><tr><th>SEGUNDA PESADA</th><th></th></tr></thead><tbody><tr><td>MOVIMIENTO INTERNO</td><td></td></tr></tbody><tbody><tr><td>BOLETO</td><td>{self.NumeroTicket}</td></tr></tbody><tbody><tr><td>2A PESADA</td><td>{self.pesoTara}</td></tr></tbody><tbody><tr><td>PESO NETO</td><td>{self.pesoNeto}</td></tr></tbody><tbody><tr><td>FECHA/HORA</td><td>{self.SFechaHora}</td></tr></tbody><tbody><tr><td>CHOFER</td><td>{self.operador_salida}</td></tr></tbody><tbody><tr><td>PESADOR</td><td>{self.usuario_segunda}</td></tr></tbody><tbody><tr><td>DESTINO</td><td>{self.destino}</td></tr></tbody><tbody><tr><td>OBSERVACIONES</td><td>{self.observaciones_salida}</td></tr></tbody>"
            except Exception as e:
                logger.exception("Falla en llenado de datos con variables")
