...
class registraSegundaPesadaVentaTortonAutomatica(QDialog):
    def __init__(self, ticket, parent=None):
        super(registraSegundaPesadaVentaTortonAutomatica, self).__init__(parent, flags=Qt.WindowCloseButtonHint)
        loadUi('Operador_Segunda_Venta_Torton_Automatica.ui', self)
        logger.debug("Operador_Segunda_Venta_Torton_Automatica")
        logger.debug("Operador registra segunda pesada Venta Automatica Torton")
        self.DB_Connect()
    ...
    def insertarDatos(self):
        ...
        # 1 ES PESADA SOLAS
        solas = '0'
        logger.debug(f"ID Destino: {id_destino}")
        ...
        try:
            query = "INSERT INTO totalco_boleto_sal(id_bol_sal, id_bol_entra, fecha_s, hora_s, peso_bruto, peso_tara, peso_neto, tipo_pesada, identifica_pedido_tipo, " \
                ...
                identifica_pedido_tipo, solas, completo, observaciones, id_destino, new_dir_w, dir_linux, imageName1, imageName2, imageName3, self.id_usuario, id_operador, id_maniobra, imageName4)
