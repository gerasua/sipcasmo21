...
class registraPrimeraDevClientTortonManual(QDialog):
    def __init__(self, fn=None, parent=None):
        super(registraPrimeraDevClientTortonManual, self).__init__(parent, flags=Qt.WindowCloseButtonHint)
        loadUi('Operador_Primera_DevCliente_Torton_Manual.ui', self)
        logger.debug("Operador_Primera_DevCliente_Torton_Manual")
        logger.debug("Operador registra primera pesada devolución de cliente Manual")
        self.DB_Connect()
    ...
    def insertarDatos(self):
        ...
        if self.impresora == "Impresora de Cinta":
            ...
            item_widget = []
            try:
                datos += f"<thead><tr><th>PRIMERA PESADA</th><th></th></tr></thead><tbody><tr><td>DEVOLUCION DE CLIENTE</td><td>&nbsp;</td></tr></tbody><tbody><tr><td>EMPRESA</td><td>{self.empresa}</td></tr></tbody><tbody><tr><td>BOLETO</td><td>{self.NumeroTicket}</td></tr></tbody><tbody><tr><td>CLIENTE</td><td>{self.cliente}</td></tr></tbody><tbody><tr><td>1A PESADA</td><td>{self.peso_insertar}</td></tr></tbody><tbody><tr><td>FECHA Y HORA</td><td>{self.fechahora}</td></tr></tbody><tbody><tr><td>PLACAS</td><td>{self.placas}</td></tr></tbody><tbody><tr><td>PRODUCTO</td><td>{self.producto}</td></tr></tbody><tbody><tr><td>CHOFER</td><td>{self.chofer}</td></tr></tbody><tbody><tr><td>PESADOR</td><td>{self.pesador}</td></tr></tbody><tbody><tr><td>OBSERVACIONES</td><td>{self.observaciones}</td></tr></tbody>"
            except Exception as e:
