...
class reimprimirTraspasoDestare(QDialog):
    def __init__(self, ticketres, parent=None):
        super(reimprimirTraspasoDestare, self).__init__(parent, flags=Qt.WindowCloseButtonHint)
        loadUi('Operador_Reimprime_Destare_Traspaso.ui', self)
        logger.debug("Operador_Reimprime_Destare_Traspaso")
        logger.debug("Operador re imprime ticket de Traspaso de sucursal Destare")
        self.DB_Connect()
        ...
        if self.portacontenedor:
            ...
            try:
                self.cur.execute("SELECT sucursal, operador_entrada, producto, fecha_primera, hora_primera, usuario_primera, observaciones_entrada, placas_portacontenedor, primera_pesada, segunda_pesada, peso_neto_embalaje, fecha_segunda, hora_segunda, destino, observaciones_salida, usuario_segunda, operador_salida, total_embalaje, peso_neto_sin_embalaje_redondeo, fecha_destare, hora_destare, observaciones, usuario_desatara, peso_neto_sin_embalaje FROM totalco_destare WHERE numero_ticket ='" + str(self.NumeroTicket) + "'")
                self.row = self.cur.fetchone()

                self.EFechaHora = str(self.row[3]) + " " + str(self.row[4])
                self.sucursal = self.row[0]
                ...
                self.Sfecha = self.row[11]
                self.SHora = self.row[12]
            ...
            try:
                self.cur.execute("SELECT sucursal, operador_entrada, producto, fecha_primera, hora_primera, usuario_primera, observaciones_entrada, placas, primera_pesada, segunda_pesada, peso_neto_embalaje, fecha_segunda, hora_segunda, destino, observaciones_salida, usuario_segunda, operador_salida, total_embalaje, peso_neto_sin_embalaje_redondeo, fecha_destare, hora_destare, observaciones, usuario_desatara, peso_neto_sin_embalaje FROM totalco_destare WHERE numero_ticket ='" + str(self.NumeroTicket) + "'")
                self.row = self.cur.fetchone()
        ...
        self.Sucursal.setText(str(self.sucursal))
        ...
        self.Destino.setText(str(self.destino))
    ...
    def insertarDatos(self):
        ...
        if self.impresora == "Impresora de Cinta":
            ...
            try:
                ...
                printer.text("Traspaso entre sucursales")
                printer.text("Boleto # {}".format(self.NumeroTicket))
                printer.text("RE IMPRESION")
                printer.text("De/Para: {}".format(self.sucursal))
    ...
    def insertarDatos3(self):
        logger.debug("Re imprimir Ticket?")
        pesador_destara = self.usario_destara
        ...
        if self.impresora == "Impresora de Cinta":
            ...
            try:
                ...
                try:
                    ...
                    logger.debug(f"Error en la variable de Observaciones {e}")
                printer.text(" ")
                try:
                    printer.text(f"Pesador: {pesador_destara}")
                    printer.text(f" ")
                    printer.text(f"___________________________")
                except Exception as e:
                    logger.exception("Error con la variable: {}".format(e))
                printer.text(" ")
    ...
    def insertarDatos4(self):
        logger.debug("Re imprimir Ticket 1a y 2a Pesada?")
        ...
        #             printer.text("Segunda Pesada: {}".format(int(self.pesoTara)))
        #         except Exception as e:
        #             logger.debug(f"Error con la variable Segunda Pesada")
        #         try:
        #             printer.text(f"Fecha/Hora: {self.Sfecha} {self.SHora}")
        #         except Exception as e:
        #             logger.exception("Error con la variable fecha: {}".format(e))
        #         try:
        #             printer.text(f"Peso Neto: {str(pesoNeto)}")
        #         except Exception as e:
        #             logger.debug(f"Error con la variable de Peso Neto: {e}")
        #         try:
        #             printer.text(f"Chofer: {str(self.operador_salida)}")
        #         except Exception as e:
        #             logger.debug(f"Error con la variable chofer: {e}")
        #         try:
        #             printer.text("Pesador: {}".format(self.usuario_segunda))
        #         except Exception as e:
        #             logger.debug(f"Error con la variable Pesador: {e}")
        #         try:
        #             printer.text("Destino: {}".format(self.destino))
        #         except Exception as e:
        #             logger.debug(f"Error con la variable Destino: {e}")
        #         try:
        #             printer.text(f"Observaciones: {str(self.observaciones_salida)}")
        #         except Exception as e:
        #             logger.debug("Error con variable observaciones: {}".format(e))
        #
        #     except Exception as e:
        #         logger.exception("Error con la impresora de tickets de la 1a y 2a Pesada: {}".format(e))
        #
        # else:
        logger.debug("Se imprimirá con Windows")
        self.documento = QTextDocument()
        ...
        item_widget = []
        try:
            datos += f"<thead><tr><th>PRIMERA PESADA</th><th></th></tr></thead><tbody><tr><td>EMPRESA</td><td>{self.empresa}</td></tr></tbody><tbody><tr><td>TRASPASO ENTRE SUCURSALES</td><td>&nbsp;</td></tr></tbody><tbody><tr><td>BOLETO</td><td>{self.NumeroTicket}</td></tr></tbody><tbody><tr><td>DE/PARA</td><td>{self.sucursal}</td></tr></tbody><tbody><tr><td>1A PESADA</td><td>{self.pesoEntrada}</td></tr></tbody><tbody><tr><td>FECHA Y HORA</td><td>{self.EFechaHora}</td></tr><tbody><tbody><tr><td>PLACAS</td><td>{self.placas_insert}</td></tr></tbody><tbody><tr><td>PRODUCTO</td><td>{self.producto}</td></tr></tbody><tbody><tr><td>CHOFER</td><td>{self.operador_entrada}</td></tr></tbody><tbody><tr><td>PESADOR</td><td>{self.usuario_primera}</td></tr></tbody><tbody><tr><td>OBSERVACIONES</td><td>{self.observaciones_entrada}</td></tr></tbody><br><br><br><thead><tr><th>SEGUNDA PESADA</th><th></th></tr></thead><tbody><tr><td>BOLETO</td><td>{self.NumeroTicket}</td></tr></tbody><tbody><tr><td>2A PESADA</td><td>{self.pesoTara}</td></tr></tbody><tbody><tr><td>PESO NETO</td><td>{self.pesoNeto}</td></tr></tbody><tbody><tr><td>FECHA/HORA</td><td>{self.SFechaHora}</td></tr></tbody><tbody><tr><td>CHOFER</td><td>{self.operador_salida}</td></tr></tbody><tbody><tr><td>PESADOR</td><td>{self.usuario_segunda}</td></tr></tbody><tbody><tr><td>DESTINO</td><td>{self.destino}</td></tr></tbody><tbody><tr><td>OBSERVACIONES</td><td>{self.observaciones_salida}</td></tr></tbody>"
        except Exception as e:
            logger.exception("Falla en llenado de datos con variables")
