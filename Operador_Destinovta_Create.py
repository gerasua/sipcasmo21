...
class operadorCreateDestinovta(QDialog):
    def __init__(self, parent=None):
        super(operadorCreateDestinovta, self).__init__(parent, flags=Qt.WindowCloseButtonHint)
        loadUi('Operador_Destinovta_Create.ui', self)
        logger.debug("Ventana agregar Destino venta")
        self.DB_Connect()
    ...
    def agregar(self):
        ...
        if self.nombre:
            ...
            try:
                query = "INSERT INTO `totalco_destino_vta` (`id_destino_vta`, `destino_vta`) VALUES (NULL, '{}')".format(
                    self.nombre)
                args = (self.nombre)
