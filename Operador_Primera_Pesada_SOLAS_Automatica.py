...
class registraPrimeraPesadaSolasAutomatica(QDialog):
    def __init__(self, fn=None, parent=None):
        super(registraPrimeraPesadaSolasAutomatica, self).__init__(parent, flags=Qt.WindowCloseButtonHint)
        loadUi('Operador_Primera_Pesada_SOLAS_Automatica.ui', self)
        logger.debug("Operador_Primera_Pesada_SOLAS_Automatica")
        logger.debug("Operador registra primera SOLAS")
        self.DB_Connect()
        self.btnr.clicked.connect(self.regresar)
        self.btn_guardar.clicked.connect(self.ObtenDatos)
        logger.debug("Inicia Primera Pesada SOLAS")
        try:
        ...
        try:
            self.cur.execute("SELECT id_portacontenedor, placas FROM `totalco_portacontenedor` WHERE caja_porta = '1' ORDER BY id_portacontenedor DESC")
            self.resultados = self.cur.fetchall()
            for resultado in self.resultados:
                self.comboBox_PlacasPorta.addItem(str(resultado[1]), int(resultado[0]))
        except Exception as e:
            logger.exception("Error For: {}".format(e))
    ...
    def insertarDatos(self):
        ...
        # Placas Portacontenedor ID
        id_placas_porta = self.comboBox_PlacasPorta.itemData(self.comboBox_PlacasPorta.currentIndex())
        placas_porta = self.comboBox_PlacasPorta.currentText()
        logger.debug("Placas Caja ID: {}".format(id_placas_porta))
        logger.debug("Placas Caja: {}".format(placas_porta))
        ...
        # LINEA TRANSPORTISTA
        try:
            self.cur.execute("SELECT trans.linea AS linea FROM totalco_portacontenedor porta INNER JOIN totalco_transporte trans ON (porta.id_transporte = trans.id_transporte) WHERE id_portacontenedor='" + str(id_placas_porta) + "'")
            self.row = self.cur.fetchone()
            self.linea = self.row[0]
        except Exception as e:
            logger.debug("Error: {}".format(e))
        ...
        try:
            query = "INSERT INTO `totalco_boleto_ent` (`id_producto`, `id_cliente`, `fecha_e`, `hora_e`, `id_tractor`, `id_operador`, `peso_e`, `tipo_pesada`, `id_usuario`, " \
                    "`completo`, `solas`, `observaciones`, `identifica_pedido_tipo`, `ruta_windows`, `ruta_linux`, `captura_cam1`, `captura_cam2`, `captura_cam3`, `id_portacontenedor`, `captura_cam4`) " \
                    "VALUES ('{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}')" \
                    "".format(id_producto, id_cliente, fecha_e, hora_e, id_tractor, id_operador, peso_insertar, self.tipo_pesada, self.id_usuario, completo, solas, observaciones,
                              identifica_pedido_tipo, new_dir_w, dir_linux, imageName1, imageName2, imageName3, id_placas_porta, imageName4)
        ...
        self.placas_porta = placas_porta
        ...
        if self.impresora == "Impresora de Cinta":
            ...
            try:
                ...
                printer.text(" ")
                printer.text("Placas Portacontenedor: {}".format(placas_porta))
                printer.text("Empresa(Cliente): {}".format(cliente))
                ...
                printer.text("Peso: {}".format(int(peso_insertar)))
            ...
            item_widget = []
            try:
                datos += f"<thead><tr><th>CERTIFICADO DE PESO</th><th></th></tr></thead><tbody><tr><td>&nbsp;</td><td>&nbsp;</td></tr></tbody><tbody><tr><td>EMPRESA</td><td>{self.empresa}</td></tr></tbody><tbody><tr><td>&nbsp;</td><td>&nbsp;</td></tr></tbody><tbody><tr><td>PLACAS PORTACONTENEDOR</td><td>{self.placas_porta}</td></tr></tbody><tbody><tr><td>EMPRESA(CLIENTE)</td><td>{self.cliente}</td></tr></tbody><tbody><tr><td>LINEA TRANSPORTISTA</td><td>{self.linea}</td></tr></tbody><tbody><tr><td>&nbsp;</td><td>&nbsp;</td></tr></tbody><tbody><tr><td>PRIMERA PESADA</td><td>&nbsp;</td></tr></tbody><tbody><tr><td>BOLETO</td><td>{self.NumeroTicket}</td></tr></tbody><tbody><tr><td>FECHA Y HORA</td><td>{self.fechahora}</td></tr></tbody><tbody><tr><td>PESO</td><td>{self.peso_insertar}</td></tr></tbody><tbody><tr><td>PRODUCTO</td><td>{self.producto}</td></tr></tbody>"
            except Exception as e:
