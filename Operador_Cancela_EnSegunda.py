...
class operadorCancelaEnSegunda(QDialog):
    def __init__(self, ticketc, parent=None):
        super(operadorCancelaEnSegunda, self).__init__(parent, flags=Qt.WindowCloseButtonHint)
        loadUi('Operador_Cancela_EnSegunda.ui', self)
        ...
        try:
            self.cur.execute("SELECT `peso_bruto`, `peso_tara`, `peso_neto` FROM totalco_boleto_sal WHERE id_bol_sal  ='" + str(self.tickeacancelar) + "'")
    ...
    def agregar(self):
        ...
        if self.InsertarObservacionesCancelar:
            ...
            try:
                query = """ UPDATE totalco_boleto_sal SET cancelado = %s, observaciones_cancelado = %s, id_usuario_cancela = %s, nombre_usuario_cancela = %s WHERE id_bol_sal = %s """
