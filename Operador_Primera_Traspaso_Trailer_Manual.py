...
class registraPrimeraTraspasoTrailerManual(QDialog):
    def __init__(self, fn=None, parent=None):
        super(registraPrimeraTraspasoTrailerManual, self).__init__(parent, flags=Qt.WindowCloseButtonHint)
        loadUi('Operador_Primera_Traspaso_Trailer_Manual.ui', self)
        logger.debug("Operador_Primera_Traspaso_Trailer_Manual")
        logger.debug("Operador registra primera traspaso manual")
        self.DB_Connect()
        ...
        # Sucursal
        try:
            self.cur.execute("SELECT * FROM totalco_sucursales WHERE id_sucursal > 0")
            self.resultados = self.cur.fetchall()
            for resultado in self.resultados:
                self.comboBox_sucursal.addItem(str(resultado[1]), int(resultado[0]))
        except Exception as e:
            logger.exception("Error For: {}".format(e))
    ...
    def insertarDatos(self):
        ...
        # Sucursal ID
        id_sucursal = self.comboBox_sucursal.itemData(self.comboBox_sucursal.currentIndex())
        sucursal = self.comboBox_sucursal.currentText()
        logger.debug("Sucursal ID: {}".format(id_sucursal))
        logger.debug("Sucursal: {}".format(sucursal))
        ...
        ticket_sucursal = self.ticket_suc.displayText()
        logger.debug("Numero Carta Porte: {}".format(ticket_sucursal))
        ...
        try:
            query = "INSERT INTO `totalco_boleto_ent` (`id_producto`, `id_sucursal`, `fecha_e`, `hora_e`, `id_tractor`, `id_operador`, `peso_e`, `tipo_pesada`, `id_usuario`, " \
                    "`completo`, `solas`, `observaciones`, `identifica_pedido_tipo`, `ruta_windows`, `ruta_linux`, `captura_cam1`, `captura_cam2`, `captura_cam3`, `ticket_sucursal`, `folio`, `id_portacontenedor`, `observaciones_manual`, `captura_cam4`) " \
                    "VALUES ('{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}')" \
                    "".format(id_producto, id_sucursal, fecha_e, hora_e, id_tractor, id_operador, int(peso_insertar), self.tipo_pesada, self.id_usuario, completo, solas, observaciones,
                              identifica_pedido_tipo, new_dir_w, dir_linux, imageName1, imageName2, imageName3, ticket_sucursal, folio, id_placas_caja, observaciones_manual, imageName4)
        ...
        if self.impresora == "Impresora de Cinta":
            ...
            item_widget = []
            try:
                datos += f"<thead><tr><th>PRIMERA PESADA</th><th></th></tr></thead><tbody><tr><td>TRASPASO ENTRE SUCURSALES</td><td>&nbsp;</td></tr></tbody><tbody><tr><td>EMPRESA</td><td>{self.empresa}</td></tr></tbody><tbody><tr><td>BOLETO</td><td>{self.NumeroTicket}</td></tr></tbody><tbody><tr><td>DE/PARA</td><td>{self.sucursal}</td></tr></tbody><tbody><tr><td>1A PESADA</td><td>{self.peso_insertar}</td></tr></tbody><tbody><tr><td>FECHA Y HORA</td><td>{self.fechahora}</td></tr></tbody><tbody><tr><td>PLACAS</td><td>{self.placas_caja}</td></tr></tbody><tbody><tr><td>PRODUCTO</td><td>{self.producto}</td></tr></tbody><tbody><tr><td>CHOFER</td><td>{self.chofer}</td></tr></tbody><tbody><tr><td>PESADOR</td><td>{self.pesador}</td></tr></tbody><tbody><tr><td>OBSERVACIONES</td><td>{self.observaciones}</td></tr></tbody>"
            except Exception as e:
