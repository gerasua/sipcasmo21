...
class reimprimirCompraDestare(QDialog):
    def __init__(self, ticketres, parent=None):
        super(reimprimirCompraDestare, self).__init__(parent, flags=Qt.WindowCloseButtonHint)
        loadUi('Operador_Reimprime_Destare_Compra.ui', self)
        logger.debug("Operador_Reimprime_Destare_Compra")
        logger.debug("Operador re imprime ticket de Compra Destare")
        self.DB_Connect()
        ...
        if self.portacontenedor:
            ...
            try:
                self.cur.execute("SELECT proveedor, operador_entrada, producto, fecha_primera, hora_primera, usuario_primera, observaciones_entrada, placas, primera_pesada, segunda_pesada, peso_neto_embalaje, fecha_segunda, hora_segunda, destino, observaciones_salida, usuario_segunda, total_embalaje, peso_neto_sin_embalaje_redondeo, fecha_destare, hora_destare, observaciones, usuario_desatara, operador_salida, peso_neto_sin_embalaje FROM totalco_destare WHERE numero_ticket ='" + str(self.NumeroTicket) + "'")
                self.row = self.cur.fetchone()
    ...
    def insertarDatos3(self):
        ...
        if self.impresora == "Impresora de Cinta":
            ...
            try:
                ...
                printer.text("Peso Neto con Destare: {}".format(self.pesoNetoRedondeado))
    ...
    <html>
        ...
        ## FINAL
        logger.debug("Aqui finaliza Destare")
        self.despuesDeInsertarDatos()
    ...
    def insertarDatos4(self):
        ...
        #         printer.text("Boleto # {}".format(self.NumeroTicket))
        #         printer.text("RE IMPRESION")
        #         printer.text("Segunda Pesada: {}".format(str(self.pesoTara)))
        #         printer.text("Peso Neto: {}".format(str(self.pesoNeto)))
        #         printer.text("Fecha/Hora: {}".format(str(self.SFechaHora)))
        #         printer.text("Chofer: {}".format(str(self.operador_salida)))
        #         printer.text("Pesador: {}".format(str(self.usuario_segunda)))
        #         printer.text("Destino: {}".format(str(self.destino)))
        #         try:
        #             printer.text("Observaciones: {}\n".format(str(self.observaciones_salida)))
        #         except Exception as e:
    ...
    <html>
        <head>
            ...
            logger.debug("SI es pesada sencilla, como llego aqui?")
            self.documento = QTextDocument()
            self.documento.clear()
