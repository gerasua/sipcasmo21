...
class operadorCreateAduana(QDialog):
    def __init__(self, parent=None):
        super(operadorCreateAduana, self).__init__(parent, flags=Qt.WindowCloseButtonHint)
        loadUi('Operador_Aduana_Create.ui', self)
        logger.debug("Ventana agregar Aduana")
        self.DB_Connect()
    ...
    def agregar(self):
        ...
        if self.nombre:
            ...
            try:
                query = "INSERT INTO `totalco_agencia_aduanal` (`id_aduana`, `nombre`) VALUES (NULL, '{}')".format(
                    self.nombre)
                args = (self.nombre)
