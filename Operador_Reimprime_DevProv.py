...
class reimprimirDevProv(QDialog):
    def __init__(self, ticketrep, parent=None):
        super(reimprimirDevProv, self).__init__(parent, flags=Qt.WindowCloseButtonHint)
        loadUi('Operador_Reimprime_DevProv.ui', self)
        logger.debug("Operador_Reimprime_DevProv")
        logger.debug("Operador re imprime ticket de Devolución a Proveedor")
        self.DB_Connect()
        ...
        if self.portacontenedor:
            ...
            try:
                self.cur.execute("SELECT pro.nombre_proveedor AS Proveedor, opee.nombre AS Chofer, prod.nombre_producto AS Producto, pri.fecha_e AS FEntrada, pri.hora_e AS HEntrada, usere.nombre_usuario AS Pesador,  pri.observaciones AS Observaciones, porta.placas AS PlacasCaja, pri.peso_e AS Peso FROM totalco_boleto_ent pri INNER JOIN totalco_proveedor pro ON (pri.id_proveedor = pro.id_proveedor) INNER JOIN totalco_operadores opee ON (pri.id_operador = opee.id_operador) INNER JOIN totalco_producto prod ON (pri.id_producto = prod.id_producto) INNER JOIN usuarios usere ON (pri.id_usuario = usere.id_usuario) INNER JOIN totalco_portacontenedor porta ON (pri.id_portacontenedor = porta.id_portacontenedor) WHERE pri.id_bol_entra ='" + str(self.NumeroTicket) + "'")
                self.row = self.cur.fetchone()
            ...
            try:
                self.cur.execute("SELECT pro.nombre_proveedor AS Proveedor, opee.nombre AS Chofer, prod.nombre_producto AS Producto, pri.fecha_e AS FEntrada, pri.hora_e AS HEntrada, usere.nombre_usuario AS Pesador, pri.observaciones AS Observaciones, trac.placas AS Placas, pri.peso_e AS Peso FROM totalco_boleto_ent pri INNER JOIN totalco_proveedor pro ON (pri.id_proveedor = pro.id_proveedor) INNER JOIN totalco_operadores opee ON (pri.id_operador = opee.id_operador) INNER JOIN totalco_producto prod ON (pri.id_producto = prod.id_producto) INNER JOIN usuarios usere ON (pri.id_usuario = usere.id_usuario) INNER JOIN totalco_tractor trac ON (pri.id_tractor = trac.id_tractor) WHERE pri.id_bol_entra ='" + str(self.NumeroTicket) + "'")
                self.row = self.cur.fetchone()
