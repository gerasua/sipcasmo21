...
class reimprimirDevCliSegunda(QDialog):
    def __init__(self, ticketres, parent=None):
        super(reimprimirDevCliSegunda, self).__init__(parent, flags=Qt.WindowCloseButtonHint)
        loadUi('Operador_Reimprime_Segunda_DevCli.ui', self)
        logger.debug("Operador_Reimprime_Segunda_DevCli")
        logger.debug("Operador re imprime ticket de Dev. de Cliente 2a Pesada")
        self.DB_Connect()
        ...
        if self.portacontenedor:
            ...
            try:
                self.cur.execute("SELECT cli.nombre_cliente AS Cliente, opee.nombre AS Chofer, prod.nombre_producto AS Producto, pri.fecha_e AS FEntrada, pri.hora_e AS HEntrada, usere.nombre_usuario AS PesadorE, pri.observaciones AS Observaciones, trac.placas AS Placas, pri.peso_e AS PrimeraP, seg.peso_tara AS SegundaP, seg.peso_neto AS PNeto, seg.fecha_s AS FSalida, seg.hora_s AS HSalida, seg.observaciones AS Observaciones, dest.destino AS Destino, users.nombre_usuario AS PesadorS, opes.nombre AS ChoferS FROM totalco_boleto_ent pri INNER JOIN totalco_cliente cli ON (pri.id_cliente = cli.id_cliente) INNER JOIN totalco_operadores opee ON (pri.id_operador = opee.id_operador) INNER JOIN totalco_producto prod ON (pri.id_producto = prod.id_producto) INNER JOIN usuarios usere ON (pri.id_usuario = usere.id_usuario) INNER JOIN totalco_tractor trac ON (pri.id_tractor = trac.id_tractor) INNER JOIN totalco_boleto_sal seg ON (pri.id_bol_entra = seg.id_bol_sal) INNER JOIN totalco_destino dest ON (seg.id_destino = dest.id_destino) INNER JOIN usuarios users ON (seg.id_usuario = users.id_usuario) INNER JOIN totalco_operadores opes ON (seg.id_operador = opes.id_operador) WHERE pri.id_bol_entra ='" + str(self.NumeroTicket) + "'")
                self.row = self.cur.fetchone()
    ...
    def insertarDatos2(self):
        ...
        if self.impresora == "Impresora de Cinta":
            ...
            item_widget = []
            try:
                datos += f"<tbody><tr><td>&nbsp;</td><td>&nbsp;</td></tr></tbody><tbody><tr><td>&nbsp;</td><td>&nbsp;</td></tr></tbody><tbody><tr><td>&nbsp;</td><td>&nbsp;</td></tr></tbody><tbody><tr><td>&nbsp;</td><td>&nbsp;</td></tr></tbody><tbody><tr><td>&nbsp;</td><td>&nbsp;</td></tr><tbody><tbody><tr><td>&nbsp;</td><td>&nbsp;</td></tr></tbody><tbody><tr><td>&nbsp;</td><td>&nbsp;</td></tr></tbody><tbody><tr><td>&nbsp;</td><td>&nbsp;</td></tr></tbody><tbody><tr><td>&nbsp;</td><td>&nbsp;</td></tr></tbody><tbody><tr><td>&nbsp;</td><td>&nbsp;</td></tr></tbody><br><br><br><thead><tr><th>SEGUNDA PESADA</th><th></th></tr></thead><tbody><tr><td>EMPRESA</td><td>{self.empresa}</td></tr></tbody><tbody><tr><td>BOLETO</td><td>{self.NumeroTicket}</td></tr></tbody><tbody><tr><td>2A PESADA</td><td>{int(self.pesoTara)}</td></tr></tbody><tbody><tr><td>PESO NETO</td><td>{int(self.pesoNeto)}</td></tr></tbody><tbody><tr><td>FECHA Y HORA</td><td>{self.SFechaHora}</td></tr></tbody><tbody><tr><td>CHOFER</td><td>{self.operador_salida}</td></tr></tbody><tbody><tr><td>CHOFER</td><td>{self.operador_salida}</td></tr></tbody><tbody><tr><td>PESADOR</td><td>{self.usuario_segunda}</td></tr></tbody><tbody><tr><td>DESTINO</td><td>{self.destino}</td></tr></tbody><tbody><tr><td>OBSERVACIONES</td><td>{self.observaciones_salida}</td></tr></tbody>"
            except Exception as e:
                logger.exception("Falla en llenado de datos con variables")
