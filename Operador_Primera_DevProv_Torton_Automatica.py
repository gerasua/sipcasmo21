...
class registraPrimeraDevProvTortonAutomatica(QDialog):
    def __init__(self, fn=None, parent=None):
        super(registraPrimeraDevProvTortonAutomatica, self).__init__(parent, flags=Qt.WindowCloseButtonHint)
        loadUi('Operador_Primera_DevProv_Torton_Automatica.ui', self)
        logger.debug("Operador_Primera_DevProv_Torton_Automatica")
        logger.debug("Operador registra primera pesada devolución a Proveedor")
        self.DB_Connect()
    ...
    def insertarDatos(self):
        ...
        try:
            query = "INSERT INTO `totalco_boleto_ent` (`id_producto`, `id_proveedor`, `fecha_e`, `hora_e`, `id_tractor`, `id_operador`, `peso_e`, `tipo_pesada`, `id_usuario`, " \
                    "`completo`, `solas`, `observaciones`, `identifica_pedido_tipo`, `ruta_windows`, `ruta_linux`, `captura_cam1`, `captura_cam2`, `captura_cam3`, `carta_porte`, `folio`, `lote`, `captura_cam4`) " \
                    "VALUES ('{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}')" \
                    "".format(id_producto, id_proveedor, fecha_e, hora_e, id_tractor, id_operador, peso_insertar, self.tipo_pesada, self.id_usuario, completo, solas, observaciones,
                              identifica_pedido_tipo, new_dir_w, dir_linux, imageName1, imageName2, imageName3,cartaPorte, folio, lote, imageName4)
        ...
        if self.impresora == "Impresora de Cinta":
            ...
            try:
                ...
                printer.text("Devolucion a Proveedor")
            ...
            item_widget = []
            try:
                datos += f"<thead><tr><th>PRIMERA PESADA</th><th></th></tr></thead><tbody><tr><td>DEVOLUCION A PROVEEDOR</td><td>&nbsp;</td></tr></tbody><tbody><tr><td>EMPRESA</td><td>{self.empresa}</td></tr></tbody><tbody><tr><td>BOLETO</td><td>{self.NumeroTicket}</td></tr></tbody><tbody><tr><td>PROVEEDOR</td><td>{self.proveedor}</td></tr></tbody><tbody><tr><td>1A PESADA</td><td>{self.peso_insertar}</td></tr></tbody><tbody><tr><td>FECHA Y HORA</td><td>{self.fechahora}</td></tr></tbody><tbody><tr><td>PLACAS</td><td>{self.placas}</td></tr></tbody><tbody><tr><td>PRODUCTO</td><td>{self.producto}</td></tr></tbody><tbody><tr><td>CHOFER</td><td>{self.chofer}</td></tr></tbody><tbody><tr><td>PESADOR</td><td>{self.pesador}</td></tr></tbody><tbody><tr><td>OBSERVACIONES</td><td>{self.observaciones}</td></tr></tbody>"
            except Exception as e:
