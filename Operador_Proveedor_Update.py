...
class operadorUpdateProveedor(QDialog):
    def __init__(self, idItem, nombreItem, parent=None):
        super(operadorUpdateProveedor, self).__init__(parent, flags=Qt.WindowCloseButtonHint)
        loadUi('Operador_Proveedor_Update.ui', self)
        logger.debug("Ventana actualizar")
        self.DB_Connect()
        logger.debug("Id: {}".format(idItem))
        logger.debug("Nombre Item: {}".format(nombreItem))

        self.cur.execute("SELECT * FROM totalco_proveedor WHERE id_proveedor='" + idItem + "'")
        self.row = self.cur.fetchone()

        self.claveProveedor = self.row[1]
        self.codigosap = self.row[2]
        logger.debug("Query: {}".format(self.cur))
        logger.debug("Result: {}".format(self.row))
        logger.debug("Codigo SAP: {}".format(self.codigosap))

        self.id = idItem
        self.lineEditUpdateClave.setText(str(self.claveProveedor))
    ...
    def actualizar(self):
        ...
        self.claveProveedorUpdate = self.lineEditUpdateClave.displayText()
        ...
        if self.itemUpdate:
            ...
            try:
                query = """ UPDATE totalco_proveedor SET nombre_proveedor = %s, clave_proveedor = %s, clave_proveedor_sap = %s WHERE id_proveedor = %s """
                data = (str(self.itemUpdate), str(self.claveProveedorUpdate), str(self.codigoSapUpdate), self.id)
