...
class registraPrimeraSalidaTortonManual(QDialog):
    def __init__(self, fn=None, parent=None):
        super(registraPrimeraSalidaTortonManual, self).__init__(parent, flags=Qt.WindowCloseButtonHint)
        loadUi('Operador_Primera_Salida_Torton_Manual.ui', self)
        logger.debug("Operador_Primera_Salida_Torton_Manual")
        logger.debug("Operador registra primera Salida Torton Manual")
        self.DB_Connect()
    ...
    def ObtenDatos(self):
        ...
        if peso_insertar != "":
            ...
            if int(peso_insertar) > 0:
                ...
                reply = QMessageBox.question(self, "Guardar", "¿Estás completamente seguro de guardar?", QMessageBox.Ok | QMessageBox.Cancel)
        ...
        if pesada_sencilla == 'NO':
            try:
                query = "INSERT INTO `totalco_boleto_ent` (`id_producto`, `id_cliente`, `fecha_e`, `hora_e`, `id_tractor`, `id_operador`, `peso_e`, `tipo_pesada`, `id_usuario`, " \
                        "`completo`, `solas`, `observaciones`, `identifica_pedido_tipo`, `ruta_windows`, `ruta_linux`, `captura_cam1`, `captura_cam2`, `captura_cam3`, `observaciones_manual`, `captura_cam4`) " \
                        "VALUES ('{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}')" \
                        "".format(id_producto, id_cliente, fecha_e, hora_e, id_tractor, id_operador, peso_insertar, self.tipo_pesada, self.id_usuario, completo, solas, observaciones,
                                  identifica_pedido_tipo, new_dir_w, dir_linux, imageName1, imageName2, imageName3, observaciones_manual, imageName4)
            ...
            if self.impresora == "Impresora de Cinta":
                ...
                try:
                    ...
                    printer.init()
                    # DATOS DEL TICKET
                    printer.text_center("{}".format(self.empresa))
                    printer.text("Boleto: {}".format(numboleto))
                    printer.text("Cliente: {}".format(cliente))
                    printer.text("Primera Pesada: {}".format(int(peso_insertar)))
                ...
                item_widget = []
                try:
                    datos += f"<thead><tr><th>PRIMERA PESADA</th><th></th></tr></thead><tbody><tr><td>EMPRESA</td><td>{self.empresa}</td></tr></tbody><tbody><tr><td>BOLETO</td><td>{self.NumeroTicket}</td></tr></tbody><tbody><tr><td>PROVEEDOR</td><td>{self.cliente}</td></tr></tbody><tbody><tr><td>1A PESADA</td><td>{self.peso_insertar}</td></tr></tbody><tbody><tr><td>FECHA Y HORA</td><td>{self.fechahora}</td></tr></tbody><tbody><tr><td>PLACAS</td><td>{self.placas}</td></tr></tbody><tbody><tr><td>PRODUCTO</td><td>{self.producto}</td></tr></tbody><tbody><tr><td>CHOFER</td><td>{self.chofer}</td></tr></tbody><tbody><tr><td>PESADOR</td><td>{self.pesador}</td></tr></tbody><tbody><tr><td>OBSERVACIONES</td><td>{self.observaciones}</td></tr></tbody>"
                except Exception as e:
            ...
            try:
                query = "INSERT INTO `totalco_boleto_ent` (`id_producto`, `id_cliente`, `fecha_e`, `hora_e`, `id_tractor`, `id_operador`, `peso_e`, `tipo_pesada`, `id_usuario`, " \
                        "`completo`, `solas`, `observaciones`, `identifica_pedido_tipo`, `ruta_windows`, `ruta_linux`, `captura_cam1`, `captura_cam2`, `captura_cam3`, `captura_cam4`) " \
                        "VALUES ('{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}')" \
                        "".format(id_producto, id_cliente, fecha_e, hora_e, id_tractor, id_operador, peso_insertar,
                                  self.tipo_pesada, self.id_usuario, completo, solas, observaciones,
            ...
            try:
                ...
                query = "INSERT INTO totalco_boleto_sal(id_bol_sal, id_bol_entra, fecha_s, hora_s, peso_bruto, peso_tara, peso_neto, tipo_pesada, identifica_pedido_tipo, " \
                    ...
                    .format(numboleto, numboleto, fecha_e, hora_e, peso_insertar, insertar_peso_tara, peso_insertar,
                            self.tipo_pesada,
                            identifica_pedido_tipo, solas, completo_destare, observaciones_empty, id_destino_vta,
                            new_dir_w, dir_linux,
