...
class registraSegundaPesadaInternoTortonAutomatica(QDialog):
    def __init__(self, ticket, parent=None):
        super(registraSegundaPesadaInternoTortonAutomatica, self).__init__(parent, flags=Qt.WindowCloseButtonHint)
        loadUi('Operador_Segunda_Interno_Torton_Automatica.ui', self)
        logger.debug("Operador_Segunda_Interno_Torton_Automatica")
        logger.debug("Operador registra segunda pesada Interno Automatica Torton")
        self.DB_Connect()
    ...
    def insertarDatos(self):
        ...
        try:
            query = "INSERT INTO totalco_boleto_sal(id_bol_sal, id_bol_entra, fecha_s, hora_s, peso_bruto, peso_tara, peso_neto, tipo_pesada, identifica_pedido_tipo, " \
                ...
                identifica_pedido_tipo, solas, completo, observaciones, destino, new_dir_w, dir_linux, imageName1, imageName2, imageName3, self.id_usuario, id_operador, id_maniobra, imageName4, id_destino)
