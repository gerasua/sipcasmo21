...
class registraPrimeraInternoTortonAutomatica(QDialog):
    def __init__(self, fn=None, parent=None):
        super(registraPrimeraInternoTortonAutomatica, self).__init__(parent, flags=Qt.WindowCloseButtonHint)
        loadUi('Operador_Primera_Interno_Torton_Automatica.ui', self)
        logger.debug("Operador_Primera_Interno_Torton_Automatica")
        logger.debug("Operador registra primer pesada interna Torton")
        self.DB_Connect()
        ...
        # Origen
        try:
            self.cur.execute("SELECT * FROM totalco_destino WHERE id_destino > 0")
            self.resultados = self.cur.fetchall()
            for resultado in self.resultados:
                self.comboBox_origen.addItem(str(resultado[1]), int(resultado[0]))
        except Exception as e:
            logger.exception("Error For: {}".format(e))
    ...
    def insertarDatos(self):
        ...
        # Origen
        id_origen = self.comboBox_origen.itemData(self.comboBox_origen.currentIndex())
        origen = self.comboBox_origen.currentText()
        logger.debug("Sucursal ID: {}".format(id_origen))
        logger.debug("Sucursal: {}".format(origen))
        ...
        try:
            query = "INSERT INTO `totalco_boleto_ent` (`id_producto`, `fecha_e`, `hora_e`, `id_tractor`, `id_operador`, `peso_e`, `tipo_pesada`, `id_usuario`, " \
                    "`completo`, `solas`, `observaciones`, `identifica_pedido_tipo`, `ruta_windows`, `ruta_linux`, `captura_cam1`, `captura_cam2`, `captura_cam3`, `origen`, `captura_cam4`, `id_origen`) " \
                    "VALUES ('{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}')" \
                    "".format(id_producto, fecha_e, hora_e, id_tractor, id_operador, peso_insertar, self.tipo_pesada, self.id_usuario, completo, solas, observaciones,
                              identifica_pedido_tipo, new_dir_w, dir_linux, imageName1, imageName2, imageName3, origen, imageName4, id_origen)
        ...
        if self.impresora == "Impresora de Cinta":
            ...
            try:
                ...
                printer.text("Origen: {}".format(origen))
            ...
            item_widget = []
            try:
                datos += f"<thead><tr><th>PRIMERA PESADA</th><th></th></tr></thead><tbody><tr><td>MOVIMIENTO INTERNO</td><td>&nbsp;</td></tr></tbody><tbody><tr><td>EMPRESA</td><td>{self.empresa}</td></tr></tbody><tbody><tr><td>BOLETO</td><td>{self.NumeroTicket}</td></tr></tbody><tbody><tr><td>ORIGEN</td><td>{self.origen}</td></tr></tbody><tbody><tr><td>1A PESADA</td><td>{self.peso_insertar}</td></tr></tbody><tbody><tr><td>FECHA Y HORA</td><td>{self.fechahora}</td></tr></tbody><tbody><tr><td>PLACAS</td><td>{self.placas}</td></tr></tbody><tbody><tr><td>PRODUCTO</td><td>{self.producto}</td></tr></tbody><tbody><tr><td>CHOFER</td><td>{self.chofer}</td></tr></tbody><tbody><tr><td>PESADOR</td><td>{self.pesador}</td></tr></tbody><tbody><tr><td>OBSERVACIONES</td><td>{self.observaciones}</td></tr></tbody>"
            except Exception as e:
