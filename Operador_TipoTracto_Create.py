...
class operadorCreateTipoTracto(QDialog):
    def __init__(self, parent=None):
        super(operadorCreateTipoTracto, self).__init__(parent, flags=Qt.WindowCloseButtonHint)
        loadUi('Operador_TipoTracto_Create.ui', self)
        logger.debug("Ventana agregar Tipo de Tracto")
        self.DB_Connect()
    ...
    def agregar(self):
        ...
        if self.nombre:
            ...
            try:
                query = "INSERT INTO `totalco_tipo_tracto` (`id_tipo_tracto`, `tipo_tracto`) VALUES (NULL, '{}')".format(
                    self.nombre)
                args = (self.nombre)
