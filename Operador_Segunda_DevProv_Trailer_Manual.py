...
class registraSegundaPesadaDevProvTrailerManual(QDialog):
    def __init__(self, ticket, parent=None):
        super(registraSegundaPesadaDevProvTrailerManual, self).__init__(parent, flags=Qt.WindowCloseButtonHint)
        loadUi('Operador_Segunda_DevProv_Trailer_Manual.ui', self)
        logger.debug("Operador_Segunda_DevProv_Trailer_Manual")
        logger.debug("Operador registra segunda pesada Dev. a Proveedor Manual Trailer")
        self.DB_Connect()
    ...
    def insertarDatos(self):
        ...
        self.pesoManual = self.Peso.displayText()
        logger.debug("LCD Manual Inicial: {}".format(self.pesoManual))
        ...
        PesoEntrada = self.pesoentrada
        logger.debug("Primera Pesada: %s" % PesoEntrada)

        PesoTara = self.pesoManual
        logger.debug("Peso Tara: {}".format(PesoTara))

        try:
            if int(PesoEntrada) > int(PesoTara):
                PesoNeto = int(PesoEntrada) - int(PesoTara)
                logger.debug("Primera pesada es mayor")
                logger.debug("Peso Neto: {}".format(int(PesoNeto)))
            else:
                PesoNeto = int(PesoTara) - int(PesoEntrada)
                logger.debug("Segunda Pesada es Mayor")
                logger.debug("Peso Neto: {}".format(int(PesoNeto)))
        except Exception as e:
        ...
        try:
            query = "INSERT INTO totalco_boleto_sal(id_bol_sal, id_bol_entra, fecha_s, hora_s, peso_bruto, peso_tara, peso_neto, tipo_pesada, identifica_pedido_tipo, " \
                    "solas, completo, observaciones, ruta_windows, ruta_linux, captura_cam1, captura_cam2, captura_cam3, id_usuario, id_operador, id_maniobra, observaciones_manual, captura_cam4) " \
                    "VALUES ('{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}')"\
                .format(self.NumeroTicket, self.NumeroTicket, fecha_s, hora_s, self.pesoentrada, PesoTara, PesoNeto, self.tipo_pesada,
                identifica_pedido_tipo, solas, completo, observaciones, new_dir_w, dir_linux, imageName1, imageName2, imageName3, self.id_usuario, id_operador, id_maniobra, observaciones_manual, imageName4)
        ...
        if self.impresora == "Impresora de Cinta":
            ...
            item_widget = []
            try:
                datos += f"<tbody><tr><td>&nbsp;</td><td>&nbsp;</td></tr></tbody><tbody><tr><td>&nbsp;</td><td>&nbsp;</td></tr></tbody><tbody><tr><td>&nbsp;</td><td>&nbsp;</td></tr></tbody><tbody><tr><td>&nbsp;</td><td>&nbsp;</td></tr></tbody><tbody><tr><td>&nbsp;</td><td>&nbsp;</td></tr><tbody><tbody><tr><td>&nbsp;</td><td>&nbsp;</td></tr></tbody><tbody><tr><td>&nbsp;</td><td>&nbsp;</td></tr></tbody><tbody><tr><td>&nbsp;</td><td>&nbsp;</td></tr></tbody><tbody><tr><td>&nbsp;</td><td>&nbsp;</td></tr></tbody><tbody><tr><td>&nbsp;</td><td>&nbsp;</td></tr></tbody><br><br><br><thead><tr><th>SEGUNDA PESADA</th><th></th></tr></thead><tbody><tr><td>BOLETO</td><td>{self.NumeroTicket}</td></tr></tbody><tbody><tr><td>2A PESADA</td><td>{self.PesoTara}</td></tr></tbody><tbody><tr><td>PESO NETO</td><td>{self.PesoNeto}</td></tr></tbody><tbody><tr><td>FECHA/HORA</td><td>{self.fechahora}</td></tr></tbody><tbody><tr><td>CHOFER</td><td>{self.chofer}</td></tr></tbody><tbody><tr><td>PESADOR</td><td>{self.pesador}</td></tr></tbody><tbody><tr><td>OBSERVACIONES</td><td>{self.observaciones}</td></tr></tbody>"
            except Exception as e:
