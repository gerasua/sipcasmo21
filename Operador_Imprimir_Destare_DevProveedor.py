...
class imprimirDestareDevProveedor(QDialog):
    def __init__(self, destarar_ticket,destarar_primeraPesada,destarar_segundaPesada,destarar_pesoNeto,pyute,ppropileno, praspa, psaco, psaco10, ptarima, pchep, ptreforzada, psacoesp, ptarimaesp, parent=None):
        super(imprimirDestareDevProveedor, self).__init__(parent, flags=Qt.WindowCloseButtonHint)
        loadUi('Operador_Imprimir_Destare_DevProveedor.ui', self)
        logger.debug("Operador_Imprimir_Destare_DevProveedor")
        logger.debug("Operador registra Destare Devolucion a Proveedor")
        self.DB_Connect()
        ...
        if self.portacontenedor:
            ...
            try:
                self.cur.execute("SELECT pri.id_bol_entra AS ticket, pro.id_proveedor AS IDPro, pro.nombre_proveedor AS NomProveedor, opee.id_operador AS IDOpeEntrada, opee.nombre AS OperadorE, opes.id_operador AS IDOpeSalida, opes.nombre AS OperadorS, trac.id_tractor AS IDTractor, trac.placas AS Placas, prod.id_producto AS IDProducto, prod.nombre_producto AS Producto, pri.fecha_e AS FEntrada, pri.hora_e AS HEntrada,seg.fecha_s AS FSalida, seg.hora_s AS HSalida, usere.id_usuario AS IDUserEntrada, usere.nombre_usuario AS UsuarioE, users.id_usuario AS IDUserSalida, users.nombre_usuario AS UsuarioS, pri.observaciones AS ObservacionesE, seg.observaciones AS ObservacionesS, seg.id_maniobra AS Maniobra, pri.tipo_pesada AS TipoPesadaEntrada, seg.tipo_pesada AS TipoPesadaSalida, trans.id_transporte AS IdTransporte, trans.linea AS Linea FROM totalco_boleto_ent pri INNER JOIN totalco_boleto_sal seg ON (pri.id_bol_entra = seg.id_bol_sal) INNER JOIN totalco_proveedor pro ON (pri.id_proveedor = pro.id_proveedor) INNER JOIN totalco_operadores opee ON (pri.id_operador = opee.id_operador) INNER JOIN totalco_operadores opes ON (seg.id_operador = opes.id_operador) INNER JOIN totalco_tractor trac ON (pri.id_tractor = trac.id_tractor) INNER JOIN totalco_producto prod ON (pri.id_producto = prod.id_producto) INNER JOIN usuarios usere ON (pri.id_usuario = usere.id_usuario) INNER JOIN usuarios users ON (seg.id_usuario = users.id_usuario) INNER JOIN totalco_transporte trans ON (trac.id_transporte = trans.id_transporte) WHERE pri.id_bol_entra ='" + str(self.NumeroTicket) + "'")
                self.row = self.cur.fetchone()
    ...
    def insertarDatos(self):
        ...
        self.id_identifica_pedido_tipo = 5
        self.identifica_pedido_tipo = "DEVOLUCION A PROVEEDOR"
        ...
        try:
            query = "INSERT INTO totalco_destare(numero_ticket, id_proveedor, proveedor, id_producto, producto, " \
                    ...
                    "usuario_desatara, id_usuario_primera, usuario_primera, id_usuario_segunda, usuario_segunda, id_portacontenedor, placas_portacontenedor," \
                    ...
                    self.id_usuario, self.pesador, self.id_usuario_primera, self.usuario_primera, self.id_usuario_segunda, self.usuario_segunda, self.id_portacontenedor, self.placas_portacontenedor,
                    self.status, self.id_lineatransportista, self.linea_transportista, self.raspa,
                                                                                                     self.ResPesoRaspa,
                                                                                                     self.saco10,
                                                                                                     self.ResSaco10,
                                                                                                     self.chep,
