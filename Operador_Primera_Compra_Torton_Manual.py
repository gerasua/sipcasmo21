...
class registraPrimeraCompraTortonManual(QDialog):
    def __init__(self, fn=None, parent=None):
        super(registraPrimeraCompraTortonManual, self).__init__(parent, flags=Qt.WindowCloseButtonHint)
        loadUi('Operador_Primera_Compra_Torton_Manual.ui', self)
        logger.debug("Operador_Primera_Compra_Torton_Manual")
        logger.debug("Operador registra primera entrada Manual")
        self.DB_Connect()
        ...
        # Placas
        try:
            self.cur.execute("SELECT * FROM totalco_tractor WHERE tipo='TORTON'")
        ...
        # Proveedores
        try:
            self.cur.execute("SELECT * FROM totalco_proveedor WHERE id_proveedor > 0")
            self.resultados = self.cur.fetchall()
            for resultado in self.resultados:
                self.comboBox_proveedor.addItem(str(resultado[3]), int(resultado[0]))
                self.comboBox_proveedor_origen.addItem(str(resultado[3]), int(resultado[0]))
        except Exception as e:
            logger.exception("Error For: {}".format(e))
    ...
    def ObtenDatos(self):
        ...
        if peso_insertar != "":
            ...
            if int(peso_insertar) > 0:
                ...
                QMessageBox.critical(self, "Mensaje", "El peso no debe ser menor a 0.", QMessageBox.Ok)
                self.Peso.setFocus()
        else:
    ...
    def insertarDatos(self):
        ...
        # Proveedor ID
        id_proveedor = self.comboBox_proveedor.itemData(self.comboBox_proveedor.currentIndex())
        proveedor = self.comboBox_proveedor.currentText()
        logger.debug("Cliente/Proveedor ID: {}".format(id_proveedor))
        logger.debug("Cliente/Proveedor: {}".format(proveedor))

        # Proveedor ID Origen
        id_proveedor_origen = self.comboBox_proveedor_origen.itemData(self.comboBox_proveedor_origen.currentIndex())
        proveedor_origen = self.comboBox_proveedor_origen.currentText()
        logger.debug("Proveedor ID Origen: {}".format(id_proveedor_origen))
        logger.debug("Proveedor Origen: {}".format(proveedor_origen))
        ...
        ##
        # Se pregunta si la pesada es sencilla
        ##
        pesada_sencilla = self.comboBox_PesadaSencilla.currentText()
        logger.debug("¿Pesada sencilla? {}".format(pesada_sencilla))
        ...
        # Se crea directorio para cḿaras
        try:
        ...
        if pesada_sencilla == 'NO':
            logger.debug("La pesada {} es sencilla".format(pesada_sencilla))
            #
            # Completo
            ...
            try:
                query = "INSERT INTO `totalco_boleto_ent` (`id_producto`, `id_proveedor`, `id_proveedor_origen`, `fecha_e`, `hora_e`, `id_tractor`, `id_operador`, `peso_e`, `tipo_pesada`, `id_usuario`, " \
                        "`completo`, `solas`, `observaciones`, `identifica_pedido_tipo`, `ruta_windows`, `ruta_linux`, `captura_cam1`, `captura_cam2`, `captura_cam3`, `carta_porte`, `folio`, `observaciones_manual`, `captura_cam4`) " \
                        "VALUES ('{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}')" \
                        "".format(id_producto, id_proveedor, id_proveedor_origen, fecha_e, hora_e, id_tractor, id_operador, int(peso_insertar), self.tipo_pesada, self.id_usuario, completo, solas, observaciones,
                                  identifica_pedido_tipo, new_dir_w, dir_linux, imageName1, imageName2, imageName3, cartaPorte, folio, observaciones_manual, imageName4)
            ...
            if self.impresora == "Impresora de Cinta":
                ...
                item_widget = []
                try:
                    datos += f"<thead><tr><th>PRIMERA PESADA</th><th></th></tr></thead><tbody><tr><td>EMPRESA</td><td>{self.empresa}</td></tr></tbody><tbody><tr><td>BOLETO</td><td>{self.NumeroTicket}</td></tr></tbody><tbody><tr><td>PROVEEDOR</td><td>{self.proveedor}</td></tr></tbody><tbody><tr><td>1A PESADA</td><td>{self.peso_insertar}</td></tr></tbody><tbody><tr><td>FECHA Y HORA</td><td>{self.fechahora}</td></tr></tbody><tbody><tr><td>PLACAS</td><td>{self.placas}</td></tr></tbody><tbody><tr><td>PRODUCTO</td><td>{self.producto}</td></tr></tbody><tbody><tr><td>CHOFER</td><td>{self.chofer}</td></tr></tbody><tbody><tr><td>PESADOR</td><td>{self.pesador}</td></tr></tbody><tbody><tr><td>OBSERVACIONES</td><td>{self.observaciones}</td></tr></tbody>"
                except Exception as e:
        ...
        ##
        # Si la pesada SI es sencilla se inserta datos en la primera y segunda pesada para pasar directamente al DESTARE
        ##
        else:
            logger.debug("La pesada {} es es sencilla".format(pesada_sencilla))
            #
            # Completo
            # La primera pesada siempre será incompleto
            # 0 = incompleto
            # 1 = completo
            completo = '1'
            # Se inserta los datos en el boleto de entrada
            try:
                query = "INSERT INTO `totalco_boleto_ent` (`id_producto`, `id_proveedor`, `id_proveedor_origen`, `fecha_e`, `hora_e`, `id_tractor`, `id_operador`, `peso_e`, `tipo_pesada`, `id_usuario`, " \
                        "`completo`, `solas`, `observaciones`, `identifica_pedido_tipo`, `ruta_windows`, `ruta_linux`, `captura_cam1`, `captura_cam2`, `captura_cam3`, `carta_porte`, `folio`, `captura_cam4`) " \
                        "VALUES ('{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}')" \
                        "".format(id_producto, id_proveedor, id_proveedor_origen, fecha_e, hora_e, id_tractor,
                                  id_operador, peso_insertar, self.tipo_pesada, self.id_usuario, completo, solas,
                                  observaciones,
                                  identifica_pedido_tipo, new_dir_w, dir_linux, imageName1, imageName2, imageName3,
                                  cartaPorte, folio, imageName4)
                logger.debug("Query: {}".format(query))
                self.cur.execute(query)
                numboleto = self.cur.lastrowid
                if self.cur.lastrowid:
                ...
                logger.debug("Error al insertar en la BD: {}".format(e))
            # Se inserta la informacion en el boleto de salida para pasar directo al destare
            try:
                id_destino = 0
                destino = " "
                id_maniobra = 0
                insertar_peso_tara = 0
                completo_destare = 1
                observaciones_empty = " "
                query = "INSERT INTO totalco_boleto_sal(id_bol_sal, id_bol_entra, fecha_s, hora_s, peso_bruto, peso_tara, peso_neto, tipo_pesada, identifica_pedido_tipo, " \
                        "solas, completo, observaciones, destino, ruta_windows, ruta_linux, captura_cam1, captura_cam2, captura_cam3, id_usuario, id_operador, id_maniobra, captura_cam4, id_destino) " \
                        "VALUES ('{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}')" \
                    .format(numboleto, numboleto, fecha_e, hora_e, peso_insertar,
                            insertar_peso_tara, peso_insertar,
                            self.tipo_pesada,
                            identifica_pedido_tipo, solas, completo_destare, observaciones_empty, destino, new_dir_w, dir_linux,
                            imageName1, imageName2, imageName3, self.id_usuario, id_operador, id_maniobra,
                            imageName4,
                            id_destino)
                logger.debug("Query: {}".format(query))
                self.cur.execute(query)
            ...
            self.chofer = operador
            self.observaciones = observaciones
            self.insertar_peso_tara = insertar_peso_tara
            ...
            if self.impresora == "Impresora de Cinta":
                ...
                try:
                    ...
                    printer.text("Fecha/Hora: {}".format(fechahora))
                    printer.text("Placas: {}".format(placas))
                    printer.text("Segunda Pesada: {}".format(insertar_peso_tara))
                    printer.text("Fecha/Hora: {}".format(fechahora))
                    printer.text("Producto: {}".format(producto))
                    printer.text("Chofer: {}".format(operador))
                    printer.text("Pesador: {}".format(self.pesador))
                ...
                item_widget = []
                try:
                    datos += f"<thead><tr><th>PRIMERA PESADA</th><th></th></tr></thead><tbody><tr><td>PESADA SENCILLA</td><td>&nbsp;</td></tr></tbody><tbody><tr><td>EMPRESA</td><td>{self.empresa}</td></tr></tbody><tbody><tr><td>BOLETO</td><td>{self.NumeroTicket}</td></tr></tbody><tbody><tr><td>PROVEEDOR</td><td>{self.proveedor}</td></tr></tbody><tbody><tr><td>1A PESADA</td><td>{self.peso_insertar}</td></tr></tbody><tbody><tr><td>FECHA Y HORA</td><td>{self.fechahora}</td></tr></tbody><tbody><tr><td>PLACAS</td><td>{self.placas}</td></tr></tbody><thead><tr><th>SEGUNDA PESADA</th><th></th></tr></thead><tbody><tr><td>2A PESADA</td><td>{self.insertar_peso_tara}</td></tr></tbody><tbody><tr><td>FECHA Y HORA</td><td>{self.fechahora}</td></tr></tbody><tbody><tr><td>PRODUCTO</td><td>{self.producto}</td></tr></tbody><tbody><tr><td>CHOFER</td><td>{self.chofer}</td></tr></tbody><tbody><tr><td>PESADOR</td><td>{self.pesador}</td></tr></tbody><tbody><tr><td>OBSERVACIONES</td><td>{self.observaciones}</td></tr></tbody>"
                except Exception as e:
