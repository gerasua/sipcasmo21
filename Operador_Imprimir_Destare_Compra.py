...
class imprimirDestareCompra(QDialog):
    def __init__(self, destarar_ticket,destarar_primeraPesada,destarar_segundaPesada,destarar_pesoNeto,pyute,ppropileno, praspa, psaco, psaco10, ptarima, pchep, ptreforzada, psacoesp, ptarimaesp, parent=None):
        super(imprimirDestareCompra, self).__init__(parent, flags=Qt.WindowCloseButtonHint)
        loadUi('Operador_Imprimir_Destare_Compra.ui', self)
        logger.debug("Operador_Imprimir_Destare_Compra")
        logger.debug("Operador registra Destare de Compra")
        self.DB_Connect()
        ...
        if self.portacontenedor:
            ...
            try:
                self.cur.execute(
                    "SELECT pri.id_bol_entra AS ticket, pro.id_proveedor AS IDPro, pro.nombre_proveedor AS NomProveedor, proori.id_proveedor AS IDProOrigen, proori.nombre_proveedor AS NomProveedorOrigen, opee.id_operador AS IDOpeEntrada, opee.nombre AS OperadorE, opes.id_operador AS IDOpeSalida, opes.nombre AS OperadorS, trac.id_tractor AS IDTractor, trac.placas AS Placas, prod.id_producto AS IDProducto, prod.nombre_producto AS Producto, pri.fecha_e AS FEntrada, pri.hora_e AS HEntrada,seg.fecha_s AS FSalida, seg.hora_s AS HSalida, usere.id_usuario AS IDUserEntrada, usere.nombre_usuario AS UsuarioE, users.id_usuario AS IDUserSalida, users.nombre_usuario AS UsuarioS, pri.observaciones AS ObservacionesE, seg.observaciones AS ObservacionesS, seg.id_destino AS IDDestino,seg.destino AS Destino, seg.id_maniobra AS Maniobra, pri.tipo_pesada AS TipoPesadaEntrada, seg.tipo_pesada AS TipoPesadaSalida, porta.id_portacontenedor AS IDPortacontenedor, porta.placas AS PlacasCaja, trans.id_transporte AS IdTransporte, trans.linea AS Linea FROM totalco_boleto_ent pri INNER JOIN totalco_boleto_sal seg ON (pri.id_bol_entra = seg.id_bol_sal) INNER JOIN totalco_proveedor pro ON (pri.id_proveedor = pro.id_proveedor) INNER JOIN totalco_proveedor proori ON (pri.id_proveedor_origen = proori.id_proveedor) INNER JOIN totalco_operadores opee ON (pri.id_operador = opee.id_operador) INNER JOIN totalco_operadores opes ON (seg.id_operador = opes.id_operador) INNER JOIN totalco_tractor trac ON (pri.id_tractor = trac.id_tractor) INNER JOIN totalco_producto prod ON (pri.id_producto = prod.id_producto) INNER JOIN usuarios usere ON (pri.id_usuario = usere.id_usuario) INNER JOIN usuarios users ON (seg.id_usuario = users.id_usuario) INNER JOIN totalco_portacontenedor porta ON (pri.id_portacontenedor = porta.id_portacontenedor) INNER JOIN totalco_transporte trans ON (trac.id_transporte = trans.id_transporte) WHERE pri.id_bol_entra ='" + str(
                        self.NumeroTicket) + "'")
                self.row = self.cur.fetchone()

                self.EFechaHora = str(self.row[13]) + " " + str(self.row[14])
                self.SFechaHora = str(self.row[15]) + " " + str(self.row[16])

                self.id_proveedor = self.row[1]
                self.proveedor = self.row[2]
                self.id_proveedor_origen = self.row[3]
                self.proveedor_origen = self.row[4]
                self.id_operador_entrada = self.row[5]
                self.operador_entrada = self.row[6]
                self.id_operador_salida = self.row[7]
                self.operador_salida = self.row[8]
                self.id_tractor = self.row[9]
                self.placas = self.row[10]
                self.id_producto = self.row[11]
                self.producto = self.row[12]
                self.fecha_primera = self.row[13]
                self.hora_primera = self.row[14]
                self.fecha_segunda = self.row[15]
                self.hora_segunda = self.row[16]
                self.id_usuario_primera = self.row[17]
                self.usuario_primera = self.row[18]
                self.id_usuario_segunda = self.row[19]
                self.usuario_segunda = self.row[20]
                self.observaciones_entrada = self.row[21]
                self.observaciones_salida = self.row[22]
                self.id_destino = self.row[23]
                self.destino = self.row[24]
                self.id_maniobra = self.row[25]
                if self.id_maniobra == 0:
                    self.maniobra = "Interna"
                else:
                    self.maniobra = "Externa"
                self.id_tipo_pesadaEntrada = self.row[26]
                if self.id_tipo_pesadaEntrada == 0:
                    self.tipo_pesadaEntrada = "Automatica"
                else:
                    self.tipo_pesadaEntrada = "Manual"
                self.id_tipo_pesadaSalida = self.row[27]
                if self.id_tipo_pesadaSalida == 0:
                    self.tipo_pesadaSalida = "Automatica"
                else:
                    self.tipo_pesadaSalida = "Manual"

                self.id_portacontenedor = self.row[28]
                self.placas_portacontenedor = self.row[29]

                self.Placas.setText(str(self.placas_portacontenedor))

                self.id_lineatransportista = self.row[30]
                self.linea_transportista = self.row[31]
            ...
            try:
                self.cur.execute("SELECT pri.id_bol_entra AS ticket, pro.id_proveedor AS IDPro, pro.nombre_proveedor AS NomProveedor, proori.id_proveedor AS IDProOrigen, proori.nombre_proveedor AS NomProveedorOrigen, opee.id_operador AS IDOpeEntrada, opee.nombre AS OperadorE, opes.id_operador AS IDOpeSalida, opes.nombre AS OperadorS, trac.id_tractor AS IDTractor, trac.placas AS Placas, prod.id_producto AS IDProducto, prod.nombre_producto AS Producto, pri.fecha_e AS FEntrada, pri.hora_e AS HEntrada,seg.fecha_s AS FSalida, seg.hora_s AS HSalida, usere.id_usuario AS IDUserEntrada, usere.nombre_usuario AS UsuarioE, users.id_usuario AS IDUserSalida, users.nombre_usuario AS UsuarioS, pri.observaciones AS ObservacionesE, seg.observaciones AS ObservacionesS, seg.id_destino AS IDDestino,seg.destino AS Destino, seg.id_maniobra AS Maniobra, pri.tipo_pesada AS TipoPesadaEntrada, seg.tipo_pesada AS TipoPesadaSalida, trans.id_transporte AS IdTransporte, trans.linea AS Linea FROM totalco_boleto_ent pri INNER JOIN totalco_boleto_sal seg ON (pri.id_bol_entra = seg.id_bol_sal) INNER JOIN totalco_proveedor pro ON (pri.id_proveedor = pro.id_proveedor) INNER JOIN totalco_proveedor proori ON (pri.id_proveedor_origen = proori.id_proveedor) INNER JOIN totalco_operadores opee ON (pri.id_operador = opee.id_operador) INNER JOIN totalco_operadores opes ON (seg.id_operador = opes.id_operador) INNER JOIN totalco_tractor trac ON (pri.id_tractor = trac.id_tractor) INNER JOIN totalco_producto prod ON (pri.id_producto = prod.id_producto) INNER JOIN usuarios usere ON (pri.id_usuario = usere.id_usuario) INNER JOIN usuarios users ON (seg.id_usuario = users.id_usuario) INNER JOIN totalco_transporte trans ON (trac.id_transporte = trans.id_transporte) WHERE pri.id_bol_entra ='" + str(self.NumeroTicket) + "'")
                self.row = self.cur.fetchone()
                ...
                self.id_lineatransportista = self.row[28]
                self.linea_transportista = self.row[29]
    ...
    def insertarDatos(self):
        ...
        self.id_identifica_pedido_tipo = 1
        self.identifica_pedido_tipo = "COMPRA"
        ...
        try:
            query = "INSERT INTO totalco_destare(numero_ticket, id_proveedor, proveedor, id_proveedor_origen, proveedor_origen, id_producto, producto, " \
                    ...
                    "usuario_desatara, id_usuario_primera, usuario_primera, id_usuario_segunda, usuario_segunda, id_destino, destino, id_portacontenedor, " \
                    "placas_portacontenedor, status, id_transportista, linea_trasnportista, raspa, total_peso_raspa, super_saco10, total_peso_saco10," \
                    ...
                    ", '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}'" \
                    ", '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}'," \
                    "'{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}')".format(self.NumeroTicket, self.id_proveedor, self.proveedor,
                    self.id_proveedor_origen, self.proveedor_origen, self.id_producto, self.producto, self.id_operador_entrada, self.operador_entrada,
                    ...
                    self.id_usuario, self.pesador, self.id_usuario_primera, self.usuario_primera, self.id_usuario_segunda, self.usuario_segunda, self.id_destino,
                    self.destino, self.id_portacontenedor, self.placas_portacontenedor, self.status, self.id_lineatransportista, self.linea_transportista,
                                                                                                     self.raspa,
                                                                                                     self.ResPesoRaspa,
                                                                                                     self.saco10,
                                                                                                     self.ResSaco10,
        ...
        if self.impresora == "Impresora de Cinta":
            ...
            try:
                ...
                try:
                    ...
                    logger.debug(f"Error en variable __ {e}")
                printer.text(f" ")
                try:
                    printer.text("Firma de conformidad")
                except Exception as e:
                    logger.debug(f"Error en variable Firma de conformidad {e}")
