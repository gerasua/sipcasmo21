...
class imprimirDestareVentaSOLAS(QDialog):
    def __init__(self, destarar_ticket,destarar_primeraPesada,destarar_segundaPesada,destarar_pesoNeto,pyute,ppropileno, praspa, psaco, psaco10, ptarima, pchep, ptreforzada, psacoesp, ptarimaesp, parent=None):
        super(imprimirDestareVentaSOLAS, self).__init__(parent, flags=Qt.WindowCloseButtonHint)
        loadUi('Operador_Imprimir_Destare_SOLAS.ui', self)
        logger.debug("Operador_Imprimir_Destare_SOLAS")
        logger.debug("Operador registra Destare de Venta SOLAS")
        self.DB_Connect()
        ...
        try:
            self.cur.execute("SELECT pri.id_bol_entra AS ticket, cli.id_cliente AS IDCliente, cli.nombre_cliente AS Cliente, opee.id_operador AS IDOpeEntrada, opee.nombre AS OperadorE, opes.id_operador AS IDOpeSalida, opes.nombre AS OperadorS, prod.id_producto AS IDProducto, prod.nombre_producto AS Producto, pri.fecha_e AS FEntrada, pri.hora_e AS HEntrada,seg.fecha_s AS FSalida, seg.hora_s AS HSalida, usere.id_usuario AS IDUserEntrada, usere.nombre_usuario AS UsuarioE, users.id_usuario AS IDUserSalida, users.nombre_usuario AS UsuarioS, pri.observaciones AS ObservacionesE, seg.observaciones AS ObservacionesS, seg.id_maniobra AS Maniobra, pri.tipo_pesada AS TipoPesadaEntrada, seg.tipo_pesada AS TipoPesadaSalida, porta.id_portacontenedor AS IDPortacontenedor, porta.placas AS Placas, cont.id_contenedor AS IDContenedor, cont.id_clave_conte AS ClaveContenedor, conttip.id_contenedor_tipo AS IDTipoContenedor, conttip.contenedor_tipo AS TipoContenedor, trans.id_transporte AS IdTransporte, trans.linea AS Linea, trac.id_tractor AS IDTractor, trac.placas AS Placas FROM totalco_boleto_ent pri INNER JOIN totalco_boleto_sal seg ON (pri.id_bol_entra = seg.id_bol_sal) INNER JOIN totalco_cliente cli ON (pri.id_cliente = cli.id_cliente) INNER JOIN totalco_operadores opee ON (pri.id_operador = opee.id_operador) INNER JOIN totalco_operadores opes ON (seg.id_operador = opes.id_operador) INNER JOIN totalco_producto prod ON (pri.id_producto = prod.id_producto) INNER JOIN usuarios usere ON (pri.id_usuario = usere.id_usuario) INNER JOIN usuarios users ON (seg.id_usuario = users.id_usuario) INNER JOIN totalco_portacontenedor porta ON (pri.id_portacontenedor = porta.id_portacontenedor) INNER JOIN totalco_contenedor cont ON (seg.id_contenedor = cont.id_contenedor) INNER JOIN totalco_contenedor_tipo conttip ON (cont.id_tipo_contenedor = conttip.id_contenedor_tipo ) INNER JOIN totalco_transporte trans ON (porta.id_transporte = trans.id_transporte) INNER JOIN totalco_tractor trac ON (pri.id_tractor = trac.id_tractor) WHERE pri.id_bol_entra ='" + str(self.NumeroTicket) + "'")
            self.row = self.cur.fetchone()

            self.EFechaHora = str(self.row[9]) + " " + str(self.row[10])
            ...
            self.id_producto = self.row[7]
            self.producto = self.row[8]
            self.fecha_primera = self.row[9]
            self.hora_primera = self.row[10]
            self.fecha_segunda = self.row[11]
            self.hora_segunda = self.row[12]
            self.id_usuario_primera = self.row[13]
            self.usuario_primera = self.row[14]
            self.id_usuario_segunda = self.row[15]
            self.usuario_segunda = self.row[16]
            self.observaciones_entrada = self.row[17]
            self.observaciones_salida = self.row[18]
            self.id_maniobra = self.row[19]
            if self.id_maniobra == 0:
                self.maniobra = "Interna"
            else:
                self.maniobra = "Externa"
            self.id_tipo_pesadaEntrada = self.row[20]
            if self.id_tipo_pesadaEntrada == 0:
                self.tipo_pesadaEntrada = "Automatica"
            else:
                self.tipo_pesadaEntrada = "Manual"
            self.id_tipo_pesadaSalida = self.row[21]
            if self.id_tipo_pesadaSalida == 0:
                self.tipo_pesadaSalida = "Automatica"
            else:
                self.tipo_pesadaSalida = "Manual"
            self.id_portacontenedor = self.row[22]
            self.placas_contenedor = self.row[23]
            self.id_contenedor = self.row[24]
            self.clave_contenedor = self.row[25]
            self.id_tipoContenedor = self.row[26]
            self.tipodeContenedor = self.row[27]
            self.id_transporte = self.row[28]
            self.linea = self.row[29]
            self.id_tractor =self.row[30]
            self.placas_tracto = self.row[31]
        ...
        self.Placas.setText(str(self.placas_contenedor))
        self.ClaveContenedor.setText(str(self.clave_contenedor))
    ...
    def insertarDatos(self):
        ...
        self.identifica_pedido_tipo = "VENTA SOLAS"
        self.solas = 1
        ...
        try:
            query = "INSERT INTO totalco_destare(numero_ticket, id_cliente, cliente, id_producto, producto,id_operador_entrada, operador_entrada, id_operador_salida, operador_salida, primera_pesada, fecha_primera,hora_primera, " \
                    "segunda_pesada, fecha_segunda, hora_segunda, peso_neto_embalaje, peso_neto_sin_embalaje, peso_neto_sin_embalaje_redondeo, yute, total_peso_yute, propileno, total_peso_propileno, " \
                    "supersaco, total_peso_supersaco, tarimas, total_peso_tarima, total_embalaje, fecha_destare, hora_destare, observaciones, observaciones_entrada, observaciones_salida," \
                    "id_tipo_pesadaEntrada, tipo_pesadaEntrada, id_tipo_pesadaSalida, tipo_pesadaSalida, id_identifica_pedido_tipo, identifica_pedido_tipo, id_maniobra, maniobra, id_usuario_destara, usuario_desatara, " \
                    "id_usuario_primera, usuario_primera, id_usuario_segunda, usuario_segunda, id_contenedor, clave_contenedor, id_portacontenedor, placas_portacontenedor, solas, status, " \
                    "id_contenedor_tipo, contenedor_tipo, id_transportista, linea_trasnportista, id_tractor, placas, raspa, total_peso_raspa, super_saco10, total_peso_saco10, " \
                    "tarimas_chep, total_peso_tarima_chep, tarimas_reforzada, total_peso_tarima_reforzada, sacos_especiales, total_peso_saco_especial, tarimas_especiales, total_peso_tarima_especial) " \
                    "VALUES " \
                    "('{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}'," \
                    "'{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}'," \
                    "'{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}'," \
                    ...
                    "'{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}'," \
                    "'{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}')"\
                    .format(self.NumeroTicket, self.id_cliente, self.cliente, self.id_producto, self.producto, self.id_operador_entrada, self.operador_entrada, self.id_operador_salida, self.operador_salida, self.pesoEntrada, self.fecha_primera, self.hora_primera,
                    self.pesoSalida, self.fecha_segunda, self.hora_segunda, self.pesoNeto, self.PesoNetoSinEmbalaje, self.PesoNetoSinEmbalajeRedondeado, self.yute, self.ResPesoYute, self.propileno, self.ResPesoPropi,
                    self.saco, self.ResSaco, self.tarima, self.ResTarima, self.ResTotal, self.fecha_destare, self.hora_destare, self.observaciones_destare, self.observaciones_entrada, self.observaciones_salida,
                    self.id_tipo_pesadaEntrada, self.tipo_pesadaEntrada, self.id_tipo_pesadaSalida, self.tipo_pesadaSalida, self.id_identifica_pedido_tipo, self.identifica_pedido_tipo, self.id_maniobra, self.maniobra, self.id_usuario, self.pesador,
                    self.id_usuario_primera, self.usuario_primera, self.id_usuario_segunda, self.usuario_segunda, self.id_contenedor, self.clave_contenedor, self.id_portacontenedor, self.placas_contenedor, self.solas, self.status,
                    self.id_tipoContenedor, self.tipodeContenedor, self.id_transporte, self.linea, self.id_tractor, self.placas_tracto, self.raspa, self.ResPesoRaspa, self.saco10, self.ResSaco10,
                    self.chep, self.ResChep, self.treforzada, self.ResTreforzada, self.sacoesp, self.ResSacoesp, self.tarimaesp, self.ResTarimaEsp)
        ...
        if self.impresora == "Impresora de Cinta":
            ...
            try:
                ...
                printer.text("Placas contenedor: {}".format(self.placas_contenedor))
                printer.text("C. Contenedor: {}".format(self.clave_contenedor))
